...
class Payment(models.Model):
    ...
    name = fields.Char(string='Serie', required=True, copy=False, readonly=True,
    ...
    date = fields.Datetime(string='Fecha de Pago', readonly=True, default=fields.Datetime.now)
    ...
    def create(self, vals_list):
        if vals_list.get('name', ('New')) == ('New'):
            vals_list['name'] = self.env['ir.sequence'].next_by_code('rifas.payment') or ('New')
    ...
    def action_approve(self):
        ...
        if self.amount != amount:
            raise ValidationError("El monto del pago no coincide con el monto de la orden de venta.")
